...
def parse_args():
    ...
    parser.add_argument('--cur_num', type=int, default=0)
...
def main():
    ...
    ckpt_path = osp.join('../../checkpoints', f'{args.pretrained_model}.pth.tar')
    ...
    ### mmdet init
    checkpoint_file = '../../checkpoints/mmdet/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth'
    ...
    for frame in tqdm(range(start, start + num_images)):

        if frame-1 >= chunk_num * cur_num and frame-1 < chunk_num * (cur_num+1):
            pass
        else:
            continue
        
        img_path = os.path.join(args.img_path, image_names[frame-1])
        ...
        for bbox_id in range(num_bbox):
            ...
            base_name = image_names[frame-1].split('.')[0]

            ## save mesh
            if args.save_mesh:
                save_path_mesh = os.path.join(args.output_folder, 'mesh')
                os.makedirs(save_path_mesh, exist_ok= True)
                save_obj(mesh, smpl_x.face, os.path.join(save_path_mesh, f'{base_name}_{bbox_id}.obj'))

            ## save single person param
            smplx_pred = {}
            smplx_pred['global_orient'] = out['smplx_root_pose'][0].cpu().numpy()
            smplx_pred['body_pose'] = out['smplx_body_pose'][0].cpu().numpy()
            smplx_pred['left_hand_pose'] = out['smplx_lhand_pose'][0].cpu().numpy()
            smplx_pred['right_hand_pose'] = out['smplx_rhand_pose'][0].cpu().numpy()
            smplx_pred['jaw_pose'] = out['smplx_jaw_pose'][0].cpu().numpy()
            smplx_pred['leye_pose'] = np.zeros((1, 3))
            smplx_pred['reye_pose'] = np.zeros((1, 3))
            smplx_pred['betas'] = out['smplx_shape'][0].cpu().numpy()
            smplx_pred['expression'] = out['smplx_expr'][0].cpu().numpy()
            smplx_pred['transl'] =  out['cam_trans'][0].cpu().numpy()
            smplx_pred['leye_pose'] = np.zeros(3)
            smplx_pred['reye_pose'] = np.zeros(3)
            ...
            smplx_pred['focal'] = focal
            smplx_pred['princpt'] = princpt
            ...
            pkl_path = os.path.join(save_path_smplx, f'{base_name}.pkl')
            ...
            with open(os.path.join(save_path_meta, f'{base_name}_{bbox_id}.json'), "w") as outfile:
