...
from assets.joint_mapping import COCO_WHOLEBODY_KEYPOINTS, SMPLX_KEYPOINTS


def project_torch(xyz, K):
    """
    xyz: [N, 3]
    K: [3, 3]
    RT: [3, 4]
...
def get_2dkps_float(bounds, K):
    corners_2d = project_torch(bounds, K)
    return corners_2d
...
def get_mapping(src: str,
    ...
    if src in __KEYPOINTS_MAPPING_CACHE__ and \
        ...
        src_names = keypoints_factory[src.lower()] # coco_wholebody
        dst_names = keypoints_factory[dst.lower()] # smplx

        dst_idxs, src_idxs, intersection = [], [], [] # empty lists for dst and src indices, intersection names
        unmapped_names, approximate_names = [], [] # lists for unmapped and approximate names
        for dst_idx, dst_name in enumerate(dst_names): # iterate through destination names 
            matched = False
            try:
                src_idx = src_names.index(dst_name) # taking the destination name and finding its index in the source names (coco_wholebody to SMPLX)
            except ValueError:
                src_idx = -1
            if src_idx >= 0:
                matched = True
                dst_idxs.append(dst_idx) # storing the index of the destination name
                src_idxs.append(src_idx) # storing the index of the source name
                intersection.append(dst_name) # storing the names when there is a match
            # approximate mapping
